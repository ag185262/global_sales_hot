...
--Script Name: d8_contract_line_xref_05.ins.sql
--Inputs:  
--Outputs:
...
--Original Query:
  --DELETE FROM contract_line_xref_ld  WHERE (contract_line_xref_id, instance_id) IN (SELECT contract_line_xref_id, instance_id     FROM ${TTMMPPDDBB1}.${AAPPLLIIDD1EENNVV}_contract_line_xref_t1     WHERE (contract_line_xref_id, instance_id) NOT IN    (SELECT contract_line_xref_id, instance_id  FROM ${TTMMPPDDBB1}.${AAPPLLIIDD1EENNVV}_contract_line_xref_t2  WHERE current_record_ind = 'D'  )     )  
   ...
   Left Join
      (
         select
            ...
            A.instance_id
         From
            ${TTMMPPDDBB1}.${AAPPLLIIDD1EENNVV}_contract_line_xref_t1   As A 
            Left Join
               (
                  SELECT
...
--Original Query:
  --LOCK TABLE  ${TTMMPPDDBB1}.${AAPPLLIIDD1EENNVV}_contract_line_xref_t1 FOR ACCESS  INSERT INTO contract_line_xref_ld (   contract_line_xref_id,  instance_id,    alt_contract_header_id,     alt_as_of_date_time,    alt_batch_nbr,  as_of_date_time,    batch_nbr,  contract_header_id,     contract_line_id,   created_by_name,    creation_date_time,     exception_flag,     for_contract_line_id,   last_update_date_time,  last_update_login_name,     last_updated_by_name,   nbr_of_items,   object1_code,   object1_id1,    object1_id2,    priced_item_flag,   tran_code,  uom_code,   upg_orig_sys_ref_id,    upg_orig_sys_ref_name ) SELECT      contract_line_xref_id,  instance_id,    alt_contract_header_id,     as_of_date_time,    batch_nbr,  as_of_date_time,    batch_nbr,  contract_header_id,     contract_line_id,   created_by_name,    creation_date_time,     exception_flag,     for_contract_line_id,   last_update_date_time,  last_update_login_name,     last_updated_by_name,   nbr_of_items,   object1_code,   object1_id1,    object1_id2,    priced_item_flag,   tran_code,  uom_code,   upg_orig_sys_ref_id,    upg_orig_sys_ref_name FROM ${TTMMPPDDBB1}.${AAPPLLIIDD1EENNVV}_contract_line_xref_t1 WHERE ( contract_line_xref_id, instance_id ) NOT IN (SELECT  contract_line_xref_id, instance_id     FROM ${TTMMPPDDBB1}.${AAPPLLIIDD1EENNVV}_contract_line_xref_t2     WHERE current_record_ind = 'D'      )  
