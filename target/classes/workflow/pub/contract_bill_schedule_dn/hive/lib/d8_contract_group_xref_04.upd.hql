...
--Script Name: d8_contract_group_xref_04.upd.sql
--Inputs:  
--Outputs:
...
--Original Query:
  --LOCK TABLE  ${AAPPLLIIDD1EENNVV}_contract_group_xref_t3 FOR ACCESS  UPDATE ${AAPPLLIIDD1EENNVV}_contract_group_xref_t2 SET current_record_ind = 'D' WHERE (contract_group_xref_id, instance_id) IN     (SELECT contract_group_xref_id, instance_id     FROM ${AAPPLLIIDD1EENNVV}_contract_group_xref_t3     GROUP BY contract_group_xref_id, instance_id     HAVING COUNT(*) = 1     )  

--Translated Query: STATUS SUCCESS (Slightly modified, added 1 more not null check to the CASE statement)
    INSERT OVERWRITE
        TABLE ${TTMMPPDDBB1}.${AAPPLLIIDD1EENNVV}_contract_group_xref_t2 SELECT
        ...
        FROM
            ${TTMMPPDDBB1}.${AAPPLLIIDD1EENNVV}_contract_group_xref_t2 AS AAPPLLIIDD1EENNVV_contract_group_xref_t2 
        Left Outer Join
            (
                SELECT
                ...
                FROM
                    ${TTMMPPDDBB1}.${AAPPLLIIDD1EENNVV}_contract_group_xref_t3      
                GROUP BY
                    contract_group_xref_id,
                    instance_id 
                HAVING
