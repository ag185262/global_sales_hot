...
--Script Name: d8_contract_group_xref_07.ins.sql
--Inputs:  
--Outputs:
...
--Original Query:
  --DELETE FROM contract_group_xref_ld  WHERE (contract_group_xref_id, instance_id) IN (SELECT contract_group_xref_id, instance_id     FROM ${TTMMPPDDBB1}.${AAPPLLIIDD1EENNVV}_contract_group_xref_t2     )  
...
--Original Query:
  --LOCK TABLE  ${TTMMPPDDBB1}.${AAPPLLIIDD1EENNVV}_contract_group_xref_t2 FOR ACCESS  INSERT INTO contract_group_xref_ld (     contract_group_xref_id,     instance_id,    alt_as_of_date_time,    alt_batch_nbr,  as_of_date_time,    batch_nbr,  created_by_name,    creation_date_time,     included_contract_group_id,     included_contract_header_id,    last_update_date_time,  last_update_login_name,     last_updated_by_name,   parent_contract_group_id,   subclass_code,  tran_code ) SELECT      contract_group_xref_id,     instance_id,    alt_as_of_date_time,    alt_batch_nbr,  as_of_date_time,    batch_nbr,  created_by_name,    creation_date_time,     included_contract_group_id,     included_contract_header_id,    last_update_date_time,  last_update_login_name,     last_updated_by_name,   parent_contract_group_id,   subclass_code,  tran_code FROM ${TTMMPPDDBB1}.${AAPPLLIIDD1EENNVV}_contract_group_xref_t2  
