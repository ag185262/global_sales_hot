...
--Script Name: d8_contract_line_svc_supp_05.ins.sql
--Inputs:  
--Outputs:
...
--Original Query:
  --DELETE FROM contract_line_svc_supp_ld WHERE  (contract_line_supp_id, instance_id) IN    (SELECT contract_line_supp_id, instance_id       FROM ${TTMMPPDDBB1}.${AAPPLLIIDD1EENNVV}_contract_line_svc_supp_t1      WHERE (contract_line_supp_id, instance_id)      NOT IN      (SELECT contract_line_supp_id, instance_id      FROM ${TTMMPPDDBB1}.${AAPPLLIIDD1EENNVV}_contract_line_svc_supp_t2      WHERE current_record_ind = 'D')    ) 
        ...
        Left Join
            (
                select
                    ...
                    A.instance_id 
                From
               ${TTMMPPDDBB1}.${AAPPLLIIDD1EENNVV}_contract_line_svc_supp_t1  As A 
                    Left Join
                    (
                        SELECT
