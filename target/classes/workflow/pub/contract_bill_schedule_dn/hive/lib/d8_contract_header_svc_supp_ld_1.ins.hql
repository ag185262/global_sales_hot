...
--Script Name: d8_contract_header_svc_supp_ld_1.ins.sql
--Inputs:  
--Outputs:
...
--Original Query:
  --DELETE FROM contract_header_svc_supp_ld  WHERE (contract_header_supp_id, instance_id) IN (SELECT contract_header_supp_id, instance_id     FROM ${TTMMPPDDBB1}.${AAPPLLIIDD1EENNVV}_contr_header_svc_supp_t2     )  
