...
insert OVERWRITE TABLE party_site_unc
SELECT
...
`edl_ingest_channel`,
`edl_ingest_time`,
row_number() over (partition by party_site_id ,instance_id
   order by last_update_date_time desc) as rnk
FROM trans_foundational_edw_gc_tedw.party_site_unc ) A
WHERE A.rnk = 1;