      ...
      export HADOOP_HEAPSIZE=32209 # Setting for HiveMetastore
      else
      ...
      HADOOP_HOME=${HADOOP_HOME:-/usr/hdp/2.6.5.1153-2/hadoop}
...
export HADOOP_CLIENT_OPTS="$HADOOP_CLIENT_OPTS"
 ...
 ALLOWED_CLI_GROUPS=(hadoop hdppltadm hdppltbnch)
ALLOWED=false
username=$(whoami)
 
for i in "${ALLOWED_CLI_GROUPS[@]}"
do
        if getent group $i | grep &>/dev/null "\b${username}\b";
        then
                ALLOWED=true
                break;
        fi
...
if [ "$SERVICE" = "cli" ] && [ $ALLOWED != true ]; then
        echo "Sorry! I have disabled hive-shell"
        exit 1
fi

export AUX_CLASSPATH=${AUX_CLASSPATH}:/data/disk1/unravel/unravel-agent/unravel-hive-1.2.0-hook.jar